        ...
        <div class="row" style="margin-left: 0.5%; margin-right: 0.5%;">
            ...
            <div class="div2 row col-lg-6 rounded  rounded bg-secondary" id="body" style="font-family: 'Times New Roman', Times, serif;">
                ...
                <div class="form-group">
                    <hr>
                    <h3 class="text-center rounded"><?php echo htmlspecialchars($lang['confirm-reservatin']); ?></h3>
                    <hr>
                    <div class="input-group mb-3 table-responsive">
                        <table class="table table-bordered table-sm">
                            ...
                            $result = $conn->query("SELECT * FROM `reserved_journey` WHERE `status`='unconfirmed' || `status`='Cancelled' limit $start_from, $num_per_page");
                                ...
                                while ($row = $result->fetch_assoc()) {
                                    ...
                                    echo "<tr> <td>$li</td> 
                                        <td>" . $row["buss_id"] . "</td> 
                                            ...
                                            <td>" . $sourse . "</td>                   
                                            <td>" . $destination . "</td>                   
                                            <td> <a href='http://localhost/TMS/process/reserv-confirm.php?r_no=$id'><button class='btn-info rounded'>". htmlspecialchars($lang['confirm_c'])."</button></a> </td> 
                                            </tr>";
                                    $li++;
                                }
                            }
                            ...
                            <tr style="text-align: center;">
                                <td colspan="10">
                                    ...
                                    $pr_query = "SELECT * FROM `reserved_journey` WHERE `status`='unconfirmed' || `status`='Cancelled'";
                                    ...
                                    if ($page > 1) {
                                        echo "<a href='http://localhost/TMS/Ticket-officer/confirm-re.php?page=" . ($page - 1) . "' class='btn btn-danger'>Previous </a>";
                                    }

                                    for ($i = 1; $i < $total_page; $i++) {
                                        echo "<a href='http://localhost/TMS/Ticket-officer/confirm-re.php?page=" . $i . "' class='btn btn-primary'>$i</a>";
                                    }

                                    if ($i > $page) {
                                        echo "<a href='http://localhost/TMS/Ticket-officer/confirm-re.php?page=" . ($page + 1) . "' class='btn btn-danger'> Next</a>";
                                    }
